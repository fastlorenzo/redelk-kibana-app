...
exports.useEditorUpdates = (services, eventEmitter, setHasUnsavedChanges, appState, savedVisInstance, visEditorController) => {
    const [isEmbeddableRendered, setIsEmbeddableRendered] = react_1.useState(false);
    const [currentAppState, setCurrentAppState] = react_1.useState();
    react_1.useEffect(() => {
        if (appState && savedVisInstance) {
            const { timefilter: { timefilter }, filterManager, } = services.data.query;
            ...
            const subscriptions = rxjs_1.merge(timefilter.getTimeUpdate$(), timefilter.getAutoRefreshFetch$(), timefilter.getFetch$(), filterManager.getFetches$()).subscribe({
            ...
            const unsubscribeStateUpdates = appState.subscribe((state) => {
                ...
                const newQuery = public_1.migrateLegacyQuery(state.query);
                if (!lodash_1.isEqual(state.query, newQuery)) {
                    appState.transitions.set('query', newQuery);
                }
                if (!lodash_1.isEqual(state.uiState, vis.uiState.getChanges())) {
