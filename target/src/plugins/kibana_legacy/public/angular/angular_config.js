...
exports.configureAppAngularModule = (angularModule, newPlatform, isLocalAngular, getHistory) => {
...
const digestOnHashChange = (getHistory) => ($rootScope) => {
    if (!getHistory)
        return;
        ...
        $httpProvider.interceptors.push(() => {
            return {
                request(opts) {
                    ...
                    if (kbnXsrfToken) {
                        safer_lodash_set_1.set(opts, ['headers', 'kbn-version'], version);
                    }
                    return opts;
                },
            };
...
const capture$httpLoadingCount = (newPlatform) => ($rootScope, $http) => {
    newPlatform.http.addLoadingCountSource(new Rx.Observable((observer) => {
        const unwatch = $rootScope.$watch(() => {
            ...
            observer.next(reqs.filter((req) => !utils_1.isSystemApiRequest(req)).length);
        });
        return unwatch;
    }));
};
