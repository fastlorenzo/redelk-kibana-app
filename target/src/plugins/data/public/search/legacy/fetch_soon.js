...
async function fetchSoon(request, options, fetchHandlers) {
    const msToDelay = fetchHandlers.config.get(common_1.UI_SETTINGS.COURIER_BATCH_SEARCHES) ? 50 : 0;
...
async function delayedFetch(request, options, fetchHandlers, ms) {
    if (ms === 0) {
        return call_client_1.callClient([request], [options], fetchHandlers)[0];
    }
