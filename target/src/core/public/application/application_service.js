...
class ApplicationService {
    ...
    setup({ context, http: { basePath }, injectedMetadata, redirectTo = (path) => {
        ...
        return {
            ...
            registerLegacyApp: (app) => {
                ...
                const mount = ({ history: appHistory }) => {
    ...
    async start({ http, overlays }) {
        ...
        const httpLoadingCount$ = new rxjs_1.BehaviorSubject(0);
        ...
        const navigateToApp = async (appId, { path, state, replace = false } = {}) => {
        ...
        return {
            applications$: applications$.pipe(operators_1.map((apps) => new Map([...apps.entries()].map(([id, app]) => [id, utils_1.getAppInfo(app)]))), operators_1.shareReplay(1)),
            capabilities,
            ...
            getUrlForApp: (appId, { path, absolute = false } = {}) => {
                ...
                return absolute ? utils_1.relativeToAbsolute(relUrl) : relUrl;
            },
            navigateToApp,
            navigateToUrl: async (url) => {
                const appInfo = utils_1.parseAppUrl(url, http.basePath, this.apps);
                if (appInfo) {
            ...
            getComponent: () => {
                ...
                return (react_1.default.createElement(ui_1.AppRouter, { history: this.history, mounters: availableMounters, "appStatuses$": applicationStatuses$, setAppLeaveHandler: this.setAppLeaveHandler, setIsMounting: (isMounting) => httpLoadingCount$.next(isMounting ? 1 : 0) }));
            },
        };
    }
